...
## ML strategy development (biotech options)

This repo’s MCP server is an execution + data adapter. For offline strategy research and machine learning, this repo also includes **optional** utilities under `src/alpaca_mcp_server/ml/` and scripts under `scripts/`.
...
### Define your biotech universe

Create a text file with one ticker per line (example: `examples/biotech_universe.txt`).

### Provide “AI input” features (optional)

If you want your model to incorporate AI-derived signals, create a CSV with:
- `underlying_symbol`: ticker
- `asof`: ISO datetime (UTC recommended)
- Any additional **numeric** columns (e.g., `ai_sentiment`, `ai_catalyst_prob`, `ai_uncertainty`)

### Build a training dataset

This produces a gzipped CSV of per-contract rows with engineered features and a label.

```bash
python3 scripts/ml_build_dataset.py \
  --universe examples/biotech_universe.txt \
  --start 2024-01-01 \
  --end 2025-01-01 \
  --out data/biotech_options_dataset.csv.gz
```

Notes:
- This builder creates a **snapshot dataset at `--end`** (and uses `--start..--end` only to compute trailing features like realized volatility).
- Default labeling is **BS-underpriced** (`label=1` when option mid is sufficiently below a Black–Scholes “fair” price using realized vol as a proxy). This is a *research heuristic*, not ground truth.
- For **true historical option backtests** (labels based on future returns), you must ensure your data source can provide option prices *as-of historical timestamps*.

### Train a model

```bash
python3 scripts/ml_train.py \
  --dataset data/biotech_options_dataset.csv.gz \
  --out models/biotech_option_model.joblib
```

### Score candidate rows

```bash
python3 scripts/ml_score.py \
  --model models/biotech_option_model.joblib \
  --rows data/biotech_options_dataset.csv.gz \
  --out data/scored.csv
```

### Mocking (tests / offline dev)

If you mock market-data calls, use the explicit mock providers in `src/alpaca_mcp_server/ml/providers/mock.py` (they generate synthetic data and **do not** call Alpaca).
